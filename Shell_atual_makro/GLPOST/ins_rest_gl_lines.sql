/*-------------------------------------------------------------------------------*/
/*                       ins_rest_gl_lines.sql                                   */
/*-------------------------------------------------------------------------------*/
/*                                                                               */
/* Contabilizar as informações das Vendas de Restaurantes para o sistema General */
/* Ledger. Gerar tabela a ser gravada no MBS-HO, com a movimentação de venda dos */
/* Restaurantes, geradas a partir do processamento dos dados originados de base  */
/* em SQL Server, sendo processadas via rotina do HONIGHT.                       */
/*                                                                               */
/* Versão 1.00 - 05/09/2011  Alcenir (3CON) - Demanda: 028242 UC 02              */
/*-------------------------------------------------------------------------------*/
...
BEGIN 
DELETE gl_interface_conecto_carga WHERE ESTABELECIMENTO = 'RESTAURANTE';
COMMIT;
...
c integer;
BEGIN
c := DBMS_HS_PASSTHROUGH.OPEN_CURSOR@restaurante;
DBMS_HS_PASSTHROUGH.PARSE@restaurante(c, 'SET SESSION SQL_MODE=''ANSI_QUOTES'';');
ret := DBMS_HS_PASSTHROUGH.EXECUTE_NON_QUERY@restaurante(c);
dbms_output.put_line(ret ||' passthrough output');
DBMS_HS_PASSTHROUGH.CLOSE_CURSOR@restaurante(c);
EXCEPTION WHEN OTHERS THEN NULL;
END;
/
ROLLBACK;
...
--           SUM(TO_NUMBER(REPLACE(V_SALE_CONT_AUT.VENDA_LIQUIDA,',','.'))) VENDA_LIQUIDA
           SUM(TO_NUMBER(V_SALE_CONT_AUT.VENDA_LIQUIDA,'999999999999.00000000000')) VENDA_LIQUIDA
      FROM V_SALE_CONT_AUT@restaurante,
     ...
     WHERE V_SALE_CONT_AUT.ID_FINALIZADORA             = GL_INTERFACE_CONECTO.ID_FINALIZADORA
       ...
       AND V_SALE_CONT_AUT.ESTABELECIMENTO             = 'RESTAURANTE'
...
DECLARE
 ...
 W_MSG_ERRO              VARCHAR2(2000)  := ' ';
  ...
  /* O processo abaixo faz busca dos dados a partir do ambiente SQL SERVER, para a tabela
     v_SALE_CONT_AUT através do dblink @restaurante, relacionando as informações para busca de
...
--     	DBMS_OUTPUT.put_line(V0001.FIELD);
        PRC$INS_rest_GL_LINES(V0001.LOJA,
                                     V0001.TRANS_CD,
                                     V0001.REP_GRP_NO,
                                     V0001.FIELD,
                                     V0001.VENDA_LIQUIDA,
    ...
    EXCEPTION
      WHEN OTHERS THEN NULL;
        V_MSG         := ('* PRC$INS_restaurante_GL_LINES - ERRO AO TENTAR EFETUAR INSERT NA TABLE: GL_INTERFACE. ' || SUBSTR(SQLERRM,1,200));
        V_COD_RETORNO := 2;
